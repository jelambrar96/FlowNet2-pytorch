dataset=$1
subset=$2
common="/home/alice/Jelambrar/ALICE/dataset"
if [ "$dataset" == "UCSDped2" ]; then
    ...
    if [[ "$subset" == "train" || "$subset" == "both" ]]; then
        ...
        do
            echo "clip ${i}"
            python3 run_flow_video.py --in_path="${in_path}/Train${i}" --out_file="${in_path}/Train${i}_full.npy" --scale 3 --recalc ${recalc} --batch 1
        done
    fi
    if [[ "$subset" == "test" || "$subset" == "both" ]]; then
        ...
        do
            echo "clip ${i}"
            python3 run_flow_video.py --in_path="${in_path}/Test${i}" --out_file="${in_path}/Test${i}_full.npy" --scale 3 --recalc ${recalc} --batch 1
        done
    fi
